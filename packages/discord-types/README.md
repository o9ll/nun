...
While it was primarily created for Nun based on Vencord and its respective packages, other client mods could also benefit from this, so it is published as a standalone package!

## Installation

```bash
npm install -D @nun/discord-types
yarn add -D @nun/discord-types
pnpm add -D @nun/discord-types
```

## Example Usage

```ts
import type { UserStore } from "@nun/discord-types";
...
```ts
import { ApplicationCommandType } from "@nun/discord-types/enums";
