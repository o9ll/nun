...
DATA_DIR="${XDG_DATA_HOME:-${HOME}/.local/share}/Nun"
...
banner() {
  echo ""
  echo -e "${CA}${B}  ╔══════════════════════════════════════════════╗${R}"
  echo -e "${CA}${B}  ║          ✦  Nun  •  Linux Installer          ║${R}"
  echo -e "${CA}${B}  ╚══════════════════════════════════════════════╝${R}"
  echo ""
}
...
fetch_json() {
  curl -fsSL -H "User-Agent: NunInstaller/1.14 (Linux)" "$RELEASE_API" \
...
download_file() {
  ...
  curl -fSL --progress-bar \
    -H "User-Agent: NunInstaller/1.14 (Linux)" \
    -o "$dest" "$url"
}
  ...
  if [ -n "$checksums_url" ]; then
    ...
    case "$ck_host" in
      github.com|*.github.com|objects.githubusercontent.com|*.githubusercontent.com)
        ...
        if curl -fsSL -H "User-Agent: NunInstaller/1.14 (Linux)" -o "$ck_tmp" "$checksums_url" 2>/dev/null; then
