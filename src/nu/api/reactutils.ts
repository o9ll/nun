...
type ElementType<T extends ReactType.FC> = T | ReactType.MemoExoticComponent<T | ReactType.ForwardRefExoticComponent<T>> | ReactType.ForwardRefExoticComponent<T> | ReactType.LazyExoticComponent<T | ReactType.MemoExoticComponent<T | ReactType.ForwardRefExoticComponent<T>> | ReactType.ForwardRefExoticComponent<T>>;
...
interface ReactUtils {
    ...
    wrapInHooks<T extends ReactType.FC>(
...
/**
 * `ReactUtils` is a utility class for interacting with React internals. Instance is accessible through the {@link NuApi}.
...
const ReactUtils: ReactUtils = {
     ...
     * @returns {any} Either the found data or `undefined`
     */
    getInternalInstance(node: HTMLElement): any {
        ...
        if (key) return node[key as keyof typeof node] as any;
        return null;
    },
    ...
    wrapElement(element: HTMLElement | HTMLElement[]) {
        return class ReactWrapper extends React.Component {
            ...
            ref: ReactType.RefObject<HTMLDivElement | null> = React.createRef();
            ...
            render() {
                return this.state.hasError ? null : React.createElement("div", {
