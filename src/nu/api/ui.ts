...
/**
 * `UI` is a utility class for creating user interfaces. Instance is accessible through the {@link NuApi}.
 * @type UI
...
const UI = {
     ...
     * @param {boolean} [options.forceShow=false] Whether to force showing the toast and ignore the NU setting
     */
    ...
    async openDialog(options: any) {
        const data = await VencordNative.nu.openDialog(options);
     ...
     * `NuApi.Components` section for details. Shown below are ones common to all setting types.
