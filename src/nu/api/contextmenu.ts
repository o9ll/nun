    ...
    static initialize() {
        ...
        if (!startupComplete) {
            ...
            for (const key in wreq.m) {
                if (Object.prototype.hasOwnProperty.call(wreq.m, key)) {
                    if (REGEX.test(wreq.m[key].toString())) {
                        menuItemsId = key;
                        break;
                    }
                }
            ...
            for (const key in wreq.m) {
                if (Object.prototype.hasOwnProperty.call(wreq.m, key)) {
                    const string = wreq.m[key].toString();
            ...
            const contextMenuComponents = wreq(menuItemsId!);
...
/**
 * `ContextMenu` is a module to help patch and create context menus. Instance is accessible through the {@link NuApi}.
 * @type ContextMenu
