...
const settings = definePluginSettings({
    uploader: {
        ...
        hidden: true,
        options: [
            { label: "Catbox (200 MB max)", value: "Catbox", default: true },
            { label: "Litterbox (temporary Catbox, 1 GB)", value: "Litterbox" },
            { label: "GoFile", value: "GoFile" },
            { label: "FileFast (fast)", value: "FileFast" },
...
async function runUploadFlow(channelId: string) {
    ...
    try {
        ...
        toast.show(`${"Uploading via"} ${getUploaderDisplayName(uploader)}…`);
        ...
        if (typeof url !== "string" || !(url.startsWith("http://") || url.startsWith("https://")))
            throw new Error("Upload completed but returned URL is invalid.");
        ...
        <Menu.MenuItem
            ...
            label={
                <div>
                    ...
                    <div>{"Upload Large File (Enhanced)"}</div>
                </div>
            }
...
function SettingsComponent() {
    ...
    const importShareXConfig = async () => {
        ...
        try {
            ...
            showToast(`${"Failed to parse JSON in sxcu file:"} ${humanError(err)}`, Toasts.Type.FAILURE);
            return;
        }
        ...
        showToast("ShareX uploader settings imported.", Toasts.Type.SUCCESS);
    };

    return (
        <div>
            <Forms.FormTitle>{"Security"}</Forms.FormTitle>
            <Forms.FormText className={Margins.bottom8}>
                {"Uploads files to an external server then shares the resulting link in the chat."}
            </Forms.FormText>
            ...
            <FormSwitch
                ...
                description={"Request confirmation before starting the upload"}
            ...
            <FormSwitch
                title={"Wrap with embeds.video (optional)"}
                description={"Wraps large video links with embeds.video (adds a third-party redirect)"}
            ...
            {uploader === "Custom" && (
                <>
                    ...
                    <div className={Margins.bottom16}>
                        <Forms.FormText>{"JSON Arguments (stored privately)"}</Forms.FormText>
                        <TextArea
                    ...
                    <div className={Margins.bottom16}>
                        <Forms.FormText>{"JSON Headers (stored privately)"}</Forms.FormText>
                        <TextArea
                    ...
                    <Button
                        onClick={() => {
                            ...
                            if (result.ok) showToast("Custom uploader settings look correct.", Toasts.Type.SUCCESS);
