...
import type { Channel, Guild } from "../types/structs";
...
class CommandManager {
    ...
    static #patchCommandSystem() {
        this.localNUBot = new DiscordModules.User({
        ...
        DiscordModules.Icons.BOT_AVATARS.betterdiscord = "https://github.com/BetterDiscord.png";
    }
    ...
    static #patchSidebarModule() {
        Patcher.after("CommandManager", DiscordModules.Sidebar, "A", (_, [props]: [{ sections: any[]; }], res: any) => {
            ...
            if (child.props?.__nuPatched) return;
            ...
            res.props.children = React.cloneElement(child, {
                renderCategoryListItem: (...args: any[]) => {
                    ...
                    if (!props.sections[args[1] - 1]?.isNU && props.sections[args[1]].isNU) {
                        return React.cloneElement(ret, {
                            children: [
                                React.createElement("hr", { className: DiscordModules.builtInSeperatorClasses.builtInSeparator }),
                                ...ret.props.children
                            ]
                        });
                    }
    ...
    static #patchIndexStore() {
        ...
        Patcher.after("CommandManager", mod, key, (_, args: any, res: any) => {
            ...
            for (const sectionedCommand of res.sectionedCommands) {
                ...
                sectionedCommand.data = sectionedCommand.data.filter((m: any) => !m.isNU);
            }
        ...
        Patcher.after("CommandManager", mod as { [key: Extract<keyof typeof mod, string>]: (o: { id: string; }) => any; }, key as Extract<keyof typeof mod, string>, (_, [{ id }]: [{ id: string; }], res: any) => {
            ...
            const Logo = ({ width, height, padding = 0, className, isSelected, selectable }: any) => {
                const wrapperClasses = [
                    selectable && DiscordModules.iconClasses.selectable,
                    isSelected && DiscordModules.iconClasses.selected,
                    DiscordModules.iconClasses.wrapper,
                    DiscordModules.iconClasses.icon,
                    className
                ].filter(Boolean).join(" ");
    ...
    static registerCommand(caller: string, command: Command) {
        ...
        const commandId = `nu-${caller}-${command.id}`;
    ...
    static async sendBotMessage(result: any, { channel }: any) {
        ...
        const loadingMessage = DiscordModules.createBotMessage({
