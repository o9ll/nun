...
let accessibilityContext: React.Context<{ reducedMotion: { enabled: false; }; }>;
function GetAccessibilityContext() {
    if (accessibilityContext) return accessibilityContext;

    accessibilityContext = DiscordModules.AccessibilityContext || React.createContext({
...
function Spinner(props: SpinnerProps) {
    const { reducedMotion } = React.useContext(GetAccessibilityContext());
    ...
    const className = React.useMemo(() => {
        return clsx({
            "spinner-stopAnimation": !animated
        }, "spinner", `spinner-${type}`, props.className);
    }, [props.className, animated, type]);

    const itemClassName = React.useMemo(() => {
        return clsx("spinner-path", props.itemClassName);
    }, [props.itemClassName]);
