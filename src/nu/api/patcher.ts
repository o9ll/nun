...
import { default as MainPatcher, type AfterCallback, type BeforeCallback, type InsteadCallback } from "../core/patcher";
...
/**
 * `Patcher` is a utility class for modifying existing functions. Instance is accessible through the {@link NuApi}.
...
class Patcher<Bounded extends boolean> {
    ...
    getPatchesByCaller(caller: string) {
        ...
        if (typeof (caller) !== "string") return Logger.err("NuApi.Patcher", "Parameter 0 of getPatchesByCaller must be a string representing the caller");
    ...
    unpatchAll(caller: string) {
        if (this.#callerName) caller = this.#callerName;
        if (typeof (caller) !== "string") return Logger.err("NuApi.Patcher", "Parameter 0 of unpatchAll must be a string representing the caller");
